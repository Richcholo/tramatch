...
class RecommendationService
{
    public function recommend(User $user, int $limit = 20): Collection
    {
        ...
        $swipes = DestinationSwipe::where('user_id', $user->id)->get()->keyBy('destination_id');
        $likedIds = $swipes
            ->where('action', 'liked')
            ->keys()
            ->all();

        if ($totalWeight === 0 || count($likedIds) === 0) {
            return collect();
        }
        ...
        return Destination::query()
            ...
            ->map(function (Destination $destination) use ($weights, $totalWeight) {
                ...
                $preferenceScore = ($matchedWeight / $totalWeight) * 100;
                $destination->match_score = round(min(100, $preferenceScore * 0.7 + 30), 2);
                $destination->matched_tags = $matchedTags->values();
                $destination->recommendation_reason = 'You liked this destination during discovery and it matches your travel profile.';
